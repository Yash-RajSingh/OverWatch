...
const getDetails = async () => {
    const request = await fetch(`https://overwatch-apis.herokuapp.com/user_details/?user_uid=${uid}`,{
        method: 'GET'
    })
    ...
    for(let x in stories){
        ...
        <h4>${identity}</h4><img src="../assets/icons/bin.png" class="mini-icon-1" onclick="deleteStory('${stories[x].suid}')"/>
            <div class="sub-card">
    ...
    for(let t in zones){
        console.log(zones[t])
        zoneCont.innerHTML += `<li class="zone-loc">${zones[t].loc_name} <img src="../assets/icons/bin.png" class="mini-icon-2" onclick="deleteZone('${zones[t].zuid}')"/></li>`
    }
}
getDetails();
...
const deleteZone = async (zid) =>{
    const request = await fetch(`https://overwatch-apis.herokuapp.com/zones/delete_zone?zuid=${zid}`, {
            method: 'DELETE'
        })
        const response = await request.json();
        console.log(response);
